...
<head>
  ...
  <title><?php echo $_SESSION["track_name"]?></title>
...
<header>
  ...
  <?php
  $sql = "SELECT * FROM " . $_SESSION["ep_name"] . " WHERE track_name = '" . $_SESSION["track_name"] . "'";
  $result = mysqli_query($con, $sql);
    ...
    while ($row = mysqli_fetch_assoc($result)) {
      ...
      <div class="container">
        ...
        <div class="SongDetails text-center">
          <h2><?php echo $row["track_name"]." - ".$row["artist1"].$row["artist2"].$row["artist3"]?></h2>
          <h5><?php echo " (".$row["version"].") " ?></h5>
        </div>
