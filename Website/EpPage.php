...
session_start();
if (isset($_GET["ep_name"])) {
  $_SESSION["ep_name"] = $_GET["ep_name"];
}
...
<body>
  ...
  <?php
  $sql = "SELECT * FROM eps WHERE ep_name = '" . $_SESSION["ep_name"] . "'";
  $result = mysqli_query($con, $sql);
    ...
    while ($row = mysqli_fetch_assoc($result)) {
      ...
      <div class="container">
        ...
        <div class="SongDetails text-center">
          <h3>
            <?php echo $row["ep_name"] . " - " . $row["artist"] ?>
          </h3>
        </div>
  ...
  <div class="container1 mt-2">
    ...
    <ol class="list-group">
      <li class="list-group-item d-flex justify-content-center bg-dark bg-opacity-25">
        <div class="Ctitle fw-bold text-black">EP TRACKS</div>
      </li>
      <?php
      $ep_name = strtolower($_SESSION["ep_name"]);
      $sql = "SELECT * FROM $ep_name ORDER BY id";
      $result = mysqli_query($con, $sql);
      //Display data
      if (mysqli_num_rows($result) > 0) {
        while ($row = mysqli_fetch_assoc($result)) {
          ...
          <li class="list-group-item d-flex justify-content-between align-items-start">
            <div class="ms-2 mt-1 mb-1 me-auto">
              <div class="fw-bold"><a
                  href="EpSongPage.php?track_name=<?php echo urlencode($row["track_name"]); ?>&ep_name=<?php echo urlencode($ep_name); ?>"
